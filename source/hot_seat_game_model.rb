class HotSeatGameModel
  ...
  def initialize (first_player_name, second_player_name)
    json = JSON.parse(File.read('config/settings.json'), {:symbolize_names => true})
    @grid = Grid.new(json[:grid_size][0], json[:grid_size][1])

    fp = Player.new(first_player_name, PlayerSettings.new(1, 'config/settings.json'))
    sp = Player.new(second_player_name, PlayerSettings.new(2, 'config/settings.json'));
